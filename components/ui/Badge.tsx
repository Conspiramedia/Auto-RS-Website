...
//   dark     — тёмный: метка «Автосалон» на плитке каталога.
//
// ПОЧЕМУ У САЛОНА ТОН dark, А НЕ СВОЙ ЦВЕТ. В верхнем ряду плитки
// каталога уже могут стоять «Аренда» (синяя) и «Просмотрено» (серая),
// а в нижнем углу — «Продвигается» (золотая). Четвёртый насыщенный
// цвет превратил бы карточку в светофор, и ни одна метка больше не
// читалась бы первой. Тёмный brand-dark — это не ещё один акцент, а
// нейтральная подпись: она сообщает КТО продаёт, тогда как цветные
// метки сообщают ЧТО с объявлением (сдаётся, продвигается).
//
// Тон переиспользует существующий 'dark' — заводить второе имя для
// той же пары цветов значило бы, что правка brand-dark однажды
// затронет одно из них и пропустит другое.
//
