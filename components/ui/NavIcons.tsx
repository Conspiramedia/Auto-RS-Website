...
// Heart — избранное. Единственный значок с ЗАЛИВКОЙ: у закладки два
// состояния, и контурное сердце от закрашенного отличается с одного
// взгляда, тогда как два контурных варианта пришлось бы различать по
// цвету — а цвет на карточке уже занят ценой и бейджами.
//
// Параметр filled, а не отдельный второй компонент: у обоих состояний
// один и тот же контур, и две копии пути разошлись бы при первой же
// правке формы.
export function HeartIcon({
  ...
  return (
    <svg
      {...STROKE}
      // Заливка — текущим цветом: снаружи сердце красится одним
      // className и в обоих состояниях, а не двумя правилами.
...
// MoreHorizontal — меню «три точки» на карточке. Три точки рисуются
// заливкой, а не обводкой: окружность радиусом 1 при strokeWidth 2
// схлопывается в пятно неопределённой формы.
