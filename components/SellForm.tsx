...
export default function SellForm({
  ...
  // СБРОС СОСТОЯНИЯ ПРИ ПЕРЕХОДЕ НА АРЕНДУ.
  //
  // Само поле у аренды скрыто (см. разметку ниже), но одного этого
  // мало: продавец мог выбрать «Битый», а потом переключить тип на
  // «Сдаю». Поле пропало бы с экрана, а значение осталось в state и
  // ушло бы в базу вместе с p_condition — арендное объявление
  // оказалось бы битым и молча выпало из выдачи под фильтр «показать
  // битые», причём продавец не понял бы почему.
  //
  // Скрытое поле не должно ничего отправлять: то, чего человек не
  // видит, он не выбирал.
  useEffect(() => {
    if (listingType !== 'sale') setCondition('normal');
  }, [listingType]);
      ...
      {step === 1 && (
        <div className="space-y-3">
          ...
          {/* СОСТОЯНИЕ АВТОМОБИЛЯ (0138).
              ...
              ТОЛЬКО ДЛЯ ПРОДАЖИ. У аренды состояние смысла не имеет:
              сдают машину на ходу и с документами, а «битую на
              запчасти» не арендуют — это не предложение проката, а
              продажа донора. Показывать поле там значило бы
              предлагать выбор, которого в жизни не бывает.

              Значение при переключении на аренду СБРАСЫВАЕТСЯ (см.
              эффект выше): скрыть поле мало — выбранное состояние
              осталось бы в state и ушло в базу, и арендное объявление
              оказалось бы «битым», молча выпав из выдачи под фильтр
              «показать битые». */}
          {listingType === 'sale' && (
          <div>
            <label className="mb-1 block text-caption text-neutral-60">
