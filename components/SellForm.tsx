    ...
    try {
      ...
      if (isEdit || signedIn) {
        ...
        contactPhone = current.session?.user.phone
          ...
          : // У аккаунта нет номера (вход по почте) — берём тот, что
            // продавец ввёл в форме. Поле ему показано именно для
            // этого случая. Пустым он быть не может: кнопка публикации
            // заблокирована до валидного номера.
            e164;
