...
//   * гость — надпись та же, «Показать номер», но ведёт на /login с
//     адресом возврата: RPC ему всё равно откажет (права выданы
//     только authenticated), а обещать в подписи вход вместо номера
//     значит отказывать раньше, чем человек попросил;
...
export default function CallSellerButton({ locale, carId, sellerId }: Props) {
  ...
  //
  // НАДПИСЬ ТА ЖЕ, ЧТО У ВОШЕДШЕГО — «Показать номер», а не «Войдите,
  // чтобы позвонить». Гость не должен решать задачу площадки: он
  // пришёл за номером, и кнопка обещает ему номер. Требование войти
  // он встретит следующим шагом, уже проявив намерение, — там оно
  // читается как условие, а не как отказ на входе.
  //
  // Действие при этом прежнее: переход на /login с возвратом сюда.
  if (userId === null) {
