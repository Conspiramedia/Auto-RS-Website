...
type Props = {
  ...
  car: {
    ...
    is_promoted?: boolean;
    // Кто подал объявление: 'dealer' — автосалон, 'private' — частник.
    // Приходит из search_cars_public (0088) — колонка там была с самого
    // начала и до сих пор до карточки не доходила. Необязательное:
    // витрина продавца (get_seller_listings) продавца не возвращает,
    // и там метка не нужна — на странице салона все объявления его.
    ...
    >
      <div className="relative aspect-[4/3] bg-surface-muted">
        ...
        <div className="pointer-events-none absolute inset-x-2 top-2 flex flex-wrap items-start gap-1.5">
          ...
          {/* «Автосалон» — свойство продавца, и стоит рядом с меткой
              аренды по той же причине: обе описывают само объявление,
              а не действия посетителя, поэтому их позиция не должна
              зависеть от того, открывал ли он карточку.
              Показывается ТОЛЬКО у салона: у частника отсутствие
              метки и означает «частное лицо». Вторая плашка «Частник»
              на каждой второй карточке стала бы фоном и перестала бы
              что-либо сообщать — ровно так же, как метка «без
              модерации» в админке рисуется лишь включённой. */}
