...
//
// На десктопе плитка вдобавок сдвинута на вторую колонку
// (xl:col-start-2), чтобы стоять в середине ряда, а не у левого края.
// Слот от этого не меняется: сдвиг решается средствами grid, а не
// позицией в потоке — иначе для четырёх колонок понадобилось бы число
// 13, которое не делится ни на 2, ни на 3 и порвало бы ряд на
// мобильном и планшете.
//
      ...
      {result.cars.length === 0 ? (
        ...
        <>
          {/* ------------------------------------------------------------
              ...
              это нормально. */}
          {/* xl:grid-flow-row-dense — пара к xl:col-start-2 у плитки
              салона. Плитка на десктопе сдвинута на вторую колонку,
              чтобы стоять в середине ряда, и без dense первая колонка
              этого ряда осталась бы пустой: обычный поток grid не
              возвращается назад к пропущенным ячейкам. С dense
              следующая карточка подтягивается в эту дыру.

              Только на xl: на двух и трёх колонках плитка не сдвинута,
              дыр не возникает, а dense там мог бы переставить карточки
              местами и разойтись с порядком сортировки выдачи. */}
