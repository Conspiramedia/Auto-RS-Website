...
export default function ProfileForm({
  ...
  return (
    ...
    // ------------------------------------------------------------
    // У частного лица справа стоит карточка аватара — она остаётся
    // второй колонкой. Но САМА ФОРМА больше не делит с ней контейнер
    // пополам: первая колонка получила фиксированные 42rem (672px),
    ...
    <div
      className={`grid gap-4 lg:items-start ${
        sellerKind === 'dealer' ? 'max-w-[42rem]' : 'lg:grid-cols-[42rem_320px]'
      }`}
    >
      ...
      {/* ------------------------------------------------------------
          ...
          НА МОБИЛЬНОМ — ПЕРВЫМ БЛОКОМ. Колонки схлопываются в одну, и
          по порядку DOM аватар оказывался под всей формой: человек
          открывал профиль и видел поля, а своё лицо — только
          прокрутив до конца. Порядок правится через order, а не
          перестановкой в разметке: на десктопе карточка обязана
          остаться во ВТОРОЙ колонке, а переставь мы её выше по DOM —
          она заняла бы первую, и поля уехали бы вправо.

          ПЛАТА ЗА ЭТО — порядок обхода с клавиатуры: order меняет
          только отрисовку, а Tab идёт по DOM, и кнопка «Изменить»
          остаётся последней. Терпимо: на мобильном клавиатурная
          навигация по форме — редкий случай, а альтернатива (перенос
          в DOM плюс явное указание колонок на десктопе) усложняет
          раскладку сильнее, чем стоит выигрыш.
