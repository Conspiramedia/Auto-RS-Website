...
export default function ProfileForm({
  ...
  return (
    ...
    // ------------------------------------------------------------
    // У частного лица рядом с формой стоит карточка аватара —
    // отдельной колонкой в 320px. САМА ФОРМА не делит с ней контейнер
    // пополам: её колонка получила фиксированные 42rem (672px),
    // ровно как списки диалогов и уведомлений (max-w-2xl у обоих).
    //
    // ПОРЯДОК КОЛОНОК: аватар первым, форма второй. Так левый край
    // карточки совпадает с началом полосы вкладок и заголовка
    // кабинета; при обратном порядке карточка начиналась с ~688px и
    // не равнялась ни с чем на странице.
    //
    // Прежде там стояло 1fr, и форма растягивалась на весь остаток
    // max-w-6xl: поля выходили заметно шире полосы вкладок над ними, и
    // разделы кабинета выглядели свёрстанными по разным правилам.
    // Теперь три раздела из четырёх имеют одну ширину контента.
    //
    // У САЛОНА АВАТАРА НЕТ, и колонка одна — но ширина та же 42rem.
    // Прежде здесь стоял max-w-3xl (768px): форма салона выходила шире
    // и полосы вкладок, и остальных разделов кабинета, то есть ровно
    // та же рассогласованность, что была у частника, только меньше.
    //
    // Парные строки («город + телефон», «часы с… до…») в 42rem
    // помещаются: с отступами карточки на колонку приходится ~300px
    // при требуемых 280px — расчёт в комментарии у самой пары ниже.
    <div
      className={`grid gap-4 lg:items-start ${
        sellerKind === 'dealer' ? 'max-w-[42rem]' : 'lg:grid-cols-[320px_42rem]'
      }`}
    >
      ...
      {/* ------------------------------------------------------------
          ЛЕВАЯ КОЛОНКА — АВАТАР. ТОЛЬКО У ЧАСТНОГО ЛИЦА.
          ------------------------------------------------------------
          Стоит сбоку, а не среди полей: это не редактируемое поле
          формы, а сведения об аккаунте, и мешать его с полями ввода
          незачем.

          ПОЧЕМУ СЛЕВА, А НЕ СПРАВА. Раньше колонка аватара была
          второй (grid-cols-[42rem_320px]), то есть начиналась примерно
          с 688px от края контейнера. Полоса вкладок кабинета при этом
          начинается с нуля — и карточка аватара висела в стороне,
          не совпадая по левому краю ни с чем на странице. Теперь
          порядок колонок обратный: карточка начинается там же, где
          вкладка «Профиль» над ней, и вертикаль страницы читается
          одной линией.

          НА УЗКОМ ЭКРАНЕ ПОРЯДОК ТОТ ЖЕ. order-first нужен и здесь:
          колонки схлопываются в одну, и по порядку DOM аватар
          оказался бы под всей формой — человек открывал профиль и
          видел поля, а своё лицо только прокрутив до конца. Прежде
          рядом стоял lg:order-none, возвращавший карточку во вторую
          колонку на десктопе; теперь возвращать некуда — она первая
          на всех ширинах, и одного order-first достаточно.

          ПЛАТА ЗА ЭТО — порядок обхода с клавиатуры: order меняет
          только отрисовку, а Tab идёт по DOM, и кнопка «Изменить»
          остаётся последней. Терпимо: клавиатурная навигация по
          форме — редкий случай, а перенос карточки выше по DOM
          заставил бы переписывать и саму форму.
