            ...
            <div>
              ...
              <div className="mt-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {/* ДВА СОСТОЯНИЯ, А НЕ ТРИ КНОПКИ.
                      Логотипа нет — одна кнопка «Загрузить логотип».
                      Логотип есть — «Заменить логотип» и «Убрать».

                      Почему «Загрузить» не остаётся рядом с «Заменить»:
                      обе открывали бы один и тот же файловый диалог и
                      писали бы в один и тот же logo.jpg — то есть
                      делали бы ровно одно и то же под разными
                      подписями. Из двух кнопок с одинаковым действием
                      человек выбирает, гадая, чем они различаются,
                      а различия нет. Файл в бакете один, и новый
                      снимок перезаписывает прежний.

                      «Убрать» стирает только ССЫЛКУ в профиле: сам
                      файл в хранилище остаётся. Давать клиенту право
                      удалять из бакета ради этого незачем — картинка
                      и так больше нигде не показывается. */}
