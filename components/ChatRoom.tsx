...
// то, что не отличает один чип от другого. Полный текст остаётся в
// title и aria-label: на кнопке сокращение, и что именно отправится,
// должно быть понятно и при наведении, и скринридеру.
//
...
// оставалось 696px, и даже после сокращения подписей шестой чип
// упирался в край. Горизонтальная прокрутка чинила симптом, но чип у
// границы читался как поломанная вёрстка.
//
// В пустой ленте места достаточно, и чипы переносятся по строкам
// (flex-wrap) — на 360px, 768px и 1280px все шесть видны целиком, без
// прокрутки. Заодно ушла лишняя полоса над клавиатурой на телефоне.
//
        ...
        <div className="mx-auto max-w-chat">
          {messages.length === 0 ? (
            /* ПУСТОЙ ЧАТ: быстрые вопросы вместо одинокой серой строки.
               ...
               Показываем только когда покупатель ещё ничего не писал
               (showQuickQuestions): продавец мог написать первым, и лента
               тогда не пуста — этот блок просто не отрисуется. */
                  ...
                  <div className="mx-auto flex max-w-md flex-wrap justify-center gap-2">
                    {QUICK_QUESTIONS.map(({ label, full }) => (
                      <button
                        ...
                        title={t(full)}
                        aria-label={t(full)}
                        className="rounded-pill bg-chat-input px-3 py-1.5 text-caption text-neutral-70 transition-colors duration-fast ease-out hover:bg-surface-muted"
                      >
                        {t(label)}
                      </button>
                    ))}
                  </div>
                  <p className="mt-4 text-caption text-neutral-50">
                    {t('chat_start_hint')}
                  </p>
                </>
              ) : (
