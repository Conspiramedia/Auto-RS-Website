...
//
// ЧТО НА САМОМ ДЕЛЕ ОГРАНИЧИВАЕТ РЯД — не ширина экрана, а
// max-w-chat = 720px (brand.chat.maxWidth): лента и панель ввода
// ограничены ею на любом мониторе, см. обёртку ниже. За вычетом px-3
// панели ряду достаётся 696px, и «влезает на десктопе» означает
// «короче 696px», а не «короче 1280px». Первая версия сокращений это
// упустила — мерили против ширины окна, и шестой чип всё равно
// обрезался.
//
// Замер в браузере (13px, системный шрифт), ширина всего ряда:
//   полные тексты            — 1723px, не влезает нигде;
//   первые сокращения (RU)   —  750px, не влезает (нужно ≤696);
//   нынешние (RU)            —  671px, влезает целиком;
//   сербские                 —  687px, влезают целиком.
// На 360px ряд по-прежнему прокручивается: шесть чипов туда не
// поместить физически.
//
