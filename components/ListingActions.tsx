...
//
// СНЯТОЕ АДМИНИСТРАТОРОМ — БЕЗ ДЕЙСТВИЙ ВООБЩЕ (миграция 0089).
// «Вернуть» у такого объявления не рисуется, потому что сервер его
// отклонит: возврат админского архива — прерогатива администратора.
// «Редактировать» тоже: правка идёт из статуса archived через возврат
// в публикацию, а этот путь закрыт, и кнопка вела бы в тупик.
// Вместо кнопок карточка показывает причину снятия и ссылку на
// поддержку — единственное, что продавец действительно может сделать.
//
...
export default function ListingActions({
  ...
  // Снятое администратором не даёт владельцу ни одного действия.
  // Пустой список вместо фильтрации отдельных кнопок: у такого
  // объявления недоступны все три (вернуть, править, продвинуть), и
  // перечислять исключения по одному значило бы разойтись с сервером
  // при следующей правке матрицы.
