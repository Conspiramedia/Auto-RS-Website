...
//
// «Редактировать» доступно у active и rejected. Это УЖЕ, чем
// принимает update_car_v3 (миграция 0067): moderation оттуда исключён
// намеренно — правка возвращает объявление в начало очереди, и
// продавец откладывал бы собственную проверку, не понимая почему.
// У проданного и архивного кнопки нет по другой причине — такое
// объявление сначала возвращают в публикацию, иначе правка меняла бы
// условия завершённой сделки задним числом.
//
...
export default function ListingActions({
  ...
  // MODERATION ИСКЛЮЧЁН НАМЕРЕННО, хотя update_car_v3 такую правку
  // принимает. Причина в её последствии: сохранение по существу
  // возвращает объявление в начало очереди на проверку, и продавец,
  // поправивший опечатку в ожидании модератора, откладывал бы
  // собственную публикацию — не понимая, почему проверка идёт второй
  // день. Правка станет доступна, когда объявление опубликуют или
  // отклонят: в первом случае это обычное редактирование, во втором —
  // единственный путь исправить замечание.
  const canEdit =
    archivedByAdmin || ['active', 'rejected'].includes(status);
