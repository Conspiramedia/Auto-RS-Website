  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 sm:py-12">
        ...
        {/* ПЕРЕДАЧА КЛЮЧЕЙ — СРАЗУ ПОД ЛИДОМ.
            ------------------------------------------------------------
            Лид говорит, что площадка соединяет продавца с покупателем,
            и снимок показывает ровно этот момент — сделку между двумя
            людьми без посредника. Дальше идут три текстовых блока
            подряд (задача, как работает, выгоды по ролям); страница
            без единой картинки читалась как устав, а не как рассказ о
            сервисе.

            Место выбрано под лидом, а не ниже: изображение работает
            иллюстрацией к обещанию, и после трёх разделов текста оно
            иллюстрировало бы уже забытую фразу.

            priority не ставится: выше стоят заголовок и лид, и ранняя
            загрузка отняла бы полосу у них. Область зарезервирована
            через aspect + fill, поэтому подгрузка не сдвигает текст
            под картинкой.

            Пропорции разные по ширине: 16/10 на узком экране и 21/9 с
            sm. Страница max-w-3xl, и высокий кадр во всю её ширину
            занял бы пол-экрана, отодвинув содержание за сгиб.

            Пустой alt: изображение декоративное, смысл несёт лид над
            ним, и озвучивать его скринридеру значит мешать. */}
