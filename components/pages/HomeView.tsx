    ...
    <>
      ...
      <main>
        ...
        {/* РАСКЛАДКА: на десктопе две колонки — текст с кнопкой слева,
            изображение справа. До lg колонка одна, и порядок сверху
            вниз: текст, картинка, кнопка.

            ПОЧЕМУ КНОПКА ВНУТРИ ТЕКСТОВОЙ КОЛОНКИ. Она относится к
            офферу, а не к картинке, и стоять должна при нём — иначе на
            широком экране повисла бы под фотографией, оторванная от
            текста, который её объясняет.

            ПОЧЕМУ 5 КОЛОНОК, А НЕ 2. Картинке отданы три доли из пяти,
            тексту две: содержание блока — три строки и кнопка, и равные
            половины оставили бы слева пустоту, а изображение сделали бы
            мельче, чем оно того стоит.

            items-center: колонки разной высоты, и без выравнивания
            короткий текст прижимался бы к верху рядом с высокой
            картинкой.

            Текст выровнен по левому краю на десктопе и по центру на
            мобильном — там он стоит над картинкой во всю ширину, и
            центровка держит блок собранным. */}
        <section className="border-t border-neutral-10 bg-surface-subtle">
          <div className="mx-auto grid max-w-6xl items-center gap-8 px-4 py-12 lg:grid-cols-5 lg:gap-12">
            <div className="text-center lg:col-span-2 lg:text-left">
              ...
              {/* Синяя кнопка (variant info), а не тёмная.
                  ...
                  означает связь и второстепенные действия: заявка
                  салона — обращение к площадке, а не публикация.

                  Кнопка стоит СРАЗУ ПОД ТЕКСТОМ на всех ширинах, а не
                  после картинки, как в других блоках страницы. Причина
                  в том, что здесь она внутри текстовой колонки: на
                  десктопе её место при оффере, а разводить порядок по
                  брейкпоинтам пришлось бы вынесением кнопки из колонки
                  наружу — то есть ломать ту самую раскладку, ради
                  которой блок и перестроен. */}
            ...
            {/* Автомобили в шоуруме: изображение отвечает смыслу блока,
                ...
                Пустой alt: изображение декоративное, смысл несут
                заголовок и кнопка рядом. */}
            <div className="relative aspect-[16/10] overflow-hidden rounded-card lg:col-span-3">
              <Image
