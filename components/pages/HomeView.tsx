          ...
          <div className="mx-auto grid max-w-6xl items-center gap-8 px-4 py-12 sm:py-16 lg:grid-cols-2 lg:gap-12">
            <div>
              {/* РАЗМЕР ЗАГОЛОВКА НА УЗКОМ ЭКРАНЕ ЗАДАН ЯВНО.
                  ...
                  clamp вместо ступени шкалы: между 320px и 550px размер
                  плавно растёт от 24px до 34px, и заголовок держится в
                  ...
                  ЗНАЧЕНИЯ ПОДОБРАНЫ ПОД ДЛИНУ ТЕКСТА. Заголовок вырос
                  до 36 символов и содержит «автомобилей» (11 букв) —
                  оно одно занимает половину строки на узком экране.

                  Нижняя граница 24px — расчётный предел, при котором
                  две строки держатся на ВСЕХ ходовых ширинах, включая
                  320px. Поднимать выше означает срыв на третью строку
                  на самых узких экранах: пробовали 25 и 26px, оба
                  ломались.

                  Заголовок при этом всего на 6 пунктов крупнее
                  подзаголовка (text-h4, 18px), и иерархия читается
                  слабее обычного. Это следствие длины фразы, а не
                  вёрстки: 36 символов со словом «автомобилей» не
                  дают уложиться в две строки крупнее.

                  Предлог «в» привязан к «Сербии» неразрывным пробелом
                  (lib/i18n): односложный предлог не должен оставаться
                  в конце строки.

                  СТУПЕНЬ sm:text-h1 УБРАНА. Она равнялась ровно 30px —
                  столько же, сколько нижняя граница clamp, — и с 640px
                  заголовок переставал расти вовсе. Хуже того, на
                  414-430px clamp давал уже 32-33px, и переход через
                  границу sm УМЕНЬШАЛ шрифт обратно до 30. Теперь clamp
                  работает до самого lg, где вступает display.
              ...
              <h1 className="max-w-2xl text-[clamp(1.5rem,6.2vw,2.125rem)] font-bold leading-tight lg:text-display">
                {t('home_hero_title')}
              </h1>
