  ...
  return (
    <>
      ...
      <main>
        ...
        {/* РАСКЛАДКА РАЗНАЯ НА ДВУХ ШИРИНАХ, И ЭТО ГЛАВНОЕ ЗДЕСЬ.
            ...
            На мобильном порядок задаётся явно (order-1/2/3), на
            десктопе сбрасывается в исходный, и колонки собираются
            обратно: текст и кнопка попадают в первую колонку сетки
            друг под другом, картинка — во вторую.

            ПОЧЕМУ 5 ДОЛЕЙ, А НЕ 2. Картинке отданы три из пяти, тексту
            две: содержание блока — три строки и кнопка, и равные
            половины оставили бы слева пустоту, а изображение сделали
            бы мельче, чем оно того стоит.

            items-center: колонки разной высоты, и без выравнивания
            короткий текст прижимался бы к верху рядом с картинкой. */}
        <section className="border-t border-neutral-10 bg-surface-subtle">
          <div className="mx-auto grid max-w-6xl items-center gap-x-12 gap-y-6 px-4 py-12 lg:grid-cols-5">
            <div className="order-1 text-center lg:col-span-2 lg:text-left">
              {/* Заголовок и подзаголовок на десктопе крупнее: блок
                  стоит в широкой колонке рядом с большой картинкой, и
                  на ступенях h3/body текст выглядел подписью к ней, а
                  не самостоятельным оффером. До lg ступени прежние —
                  там текст и так во всю ширину экрана. */}
              ...
              <p className="mt-2 text-caption text-neutral-50 lg:text-body">
            ...
            {/* Автомобили в шоуруме: изображение отвечает смыслу блока,
                ...
                правой колонки, иначе кнопка из первой колонки растянула
                бы сетку и изображение встало бы только напротив текста.
            ...
            <div className="order-3 flex justify-center lg:col-span-2 lg:col-start-1 lg:justify-start">
              <Button
                variant="info"
                size="lg"
