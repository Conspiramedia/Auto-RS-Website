...
export default function MyListingCard({ locale, listing }: Props) {
  ...
  return (
    <Card padding="sm" className="flex h-full flex-col">
      ...
      // mt-3 — ЗАЗОР НАД РАЗДЕЛИТЕЛЕМ. Линия border-t раньше имела
      // отступ только снизу (pt-3), и у объявления с плашкой причины
      // отклонения она вставала вплотную к её нижнему краю: красный
      // блок выглядел подчёркнутым, а не отделённым от метрик.
      //
      // Отступ стоит здесь, а не у плашек: перед этой линией может
      // оказаться любой из блоков выше (отклонение, снятие
      // администратором, истечение срока) или ни одного, и задавать
      // зазор каждому — значит однажды забыть про новый.
