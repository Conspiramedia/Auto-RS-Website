...
--
-- ПОИСК — ТОТ ЖЕ, ЧТО У КАТАЛОГА. p_query прогоняется через
-- f_normalize (lower + unaccent) и ищется триграммами плюс
-- подстрокой — ровно как в 0008/0060. Своя нормализация здесь
-- означала бы, что админка ищет не так, как сайт, и модератор не
-- находит то, на что смотрит покупатель.
--
-- ЧТО ЭТО ДАЁТ И ЧЕГО НЕ ДАЁТ. Снимается диакритика и регистр:
-- «skoda», «Skoda» и «Škoda» — одно и то же, «Beograd» находит
-- «beograd». Транслитерации между алфавитами НЕТ: «Шкода» не найдёт
-- «Škoda», а «БМВ» не найдёт «BMW» — f_normalize кириллицу в
-- латиницу не переводит. Это известное ограничение всего поиска
-- площадки (зафиксировано в RELEASE.md, раздел «Отложено
-- намеренно»), и чинить его надо разом для каталога, приложения и
-- админки, а не отдельной таблицей соответствий здесь.
--
