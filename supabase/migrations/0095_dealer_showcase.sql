...
-- ============================================================
-- ВНИМАНИЕ: КОД ВИТРИНЫ ОТКАЧЕН, А ЭТА МИГРАЦИЯ — НЕТ.
-- ------------------------------------------------------------
-- Экран /my/showcase, компонент DealerShowcaseCard и блок салонов на
-- главной были откачены (см. DONE.md). Схема осталась применённой, и
-- файл лежит здесь намеренно, а не по недосмотру.
--
-- Причина техническая: база помнит применённую 0095 в своей истории
-- миграций. Удали мы файл — supabase db push отказывается работать
-- вовсе («Remote migration versions not found in local migrations
-- directory»), и следующая миграция не пройдёт, пока расхождение не
-- устранено.
--
-- Причина по существу: изменения аддитивны и никому не мешают. Четыре
-- nullable-колонки без данных и две функции с расширенными, но
-- обратно совместимыми сигнатурами ведут себя ровно так же, как до
-- них, — и сайт, и приложение работают как раньше. Снимать их
-- обратной миграцией значило бы потерять содержимое полей, если салон
-- успел их заполнить; если витрину вернём, схема уже готова.
--
-- ЧТО ЭТО ЗНАЧИТ ДЛЯ ЧИТАТЕЛЯ: колонки и функции ниже существуют в
-- проде, но НИ ОДИН клиент их сейчас не читает и не пишет.
-- ------------------------------------------------------------
--
