...
Deno.serve(async (_req: Request) => {
  try {
    ...
    const tasks = [
      ...
      'expire_listings',
      // Уровни продавцов (0143). ПОСЛЕ expire_listings намеренно: та
      // задача уводит просроченные объявления из active, и уровень
      // обязан считаться уже по новому составу — иначе продавец сутки
      // носил бы ступень за объявления, которых в выдаче нет.
      //
      // Пересчитываются только помеченные профили, поэтому обычный
      // запуск разбирает единицы строк. Возвращает их число.
      'recalc_seller_tiers',
    ];
