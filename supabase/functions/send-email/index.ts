...
//
// ------------------------------------------------------------
// ЗАПУСК: по расписанию раз в 5 минут либо вручную HTTP-запросом.
// Расписание живёт в pg_cron (задание send-email-job, миграция 0134),
// а НЕ в панели: вкладки Edge Functions → Schedules в этом проекте
// нет, и указание на неё однажды уже стоило недели неотправленных
// писем. Параллельные запуски безопасны: claim_email_batch использует
// FOR UPDATE SKIP LOCKED, поэтому одно письмо не уйдёт дважды.
//
// ПОЧЕМУ ПЯТЬ МИНУТ. Письмо тяжелее пуша — отдельный исходящий запрос
// к Resend на каждое сообщение, — а задержка для него некритична:
// решение модерации не ждут секундами, в отличие от уведомления о
// новом сообщении в чате, ради которого send-push ходит раз в минуту.
// Коды входа по почте этим интервалом не затронуты: они уходят
// синхронно через auth-email-hook, минуя очередь.
//
// ------------------------------------------------------------
