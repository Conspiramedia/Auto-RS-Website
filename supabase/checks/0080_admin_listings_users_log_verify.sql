...
-- ЧТО ПРОВЕРИТЬ:
--
--   а) Поиск. /admin/listings, в поиск «skoda» — находятся Škoda
--      (диакритика и регистр снимаются). Сверить с каталогом:
--      админка обязана находить то же, что и покупатель.
--      ВНИМАНИЕ: «Шкода» кириллицей НЕ найдёт «Škoda» — транслитерации
--      между алфавитами в f_normalize нет. Это известное ограничение
--      всего поиска площадки (RELEASE.md, «Отложено намеренно»), а не
--      дефект админки.
--
