...
// («Volkswagen Golf», «Audi do 16.000 €», «Dizel»), собранные из
// РЕАЛЬНЫХ данных каталога. Подсказка, ведущая в пустую выдачу, хуже
...
// по-разному устойчивы к продаже отдельной машины: топливо переживает
// что угодно, а конкретная пара «марка+модель» может уйти вся.
// Подробнее — в шапке миграции 0073.
...
// ------------------------------------------------------------
// Подписи топлива. Продублированы из lib/types.ts (FUELS) намеренно:
// генератор — обычный .mjs без сборщика, и импортировать .ts из него
// нельзя без транспиляции. Набор из пяти значений меняется примерно
// никогда, а рассинхрон ловится проверкой ниже: значение, которого
// здесь нет, отбрасывается с предупреждением, а не молча.
// ------------------------------------------------------------
const FUEL_LABELS = {
...
function toSuggestion(row) {
  ...
  if (row.kind === 'fuel') {
    const label = FUEL_LABELS[row.fuel];
    if (!label) {
      console.warn(
        `  · пропущено топливо «${row.fuel}»: нет подписи в FUEL_LABELS ` +
          '(рассинхрон с lib/types.ts?)',
      );
      return null;
    }
    return { text: { ...label }, filters: { fuel: row.fuel } };
  }
...
async function main() {
  ...
  // задала сама RPC (order by count desc).
  const byKind = { brand_model: [], brand_price: [], fuel: [] };
  for (const row of rows) {
    const suggestion = toSuggestion(row);
    if (suggestion && byKind[row.kind]) byKind[row.kind].push(suggestion);
  }

  // Топливо ставим первым в круге: этих фраз всего пять, и без
  // приоритета они утонули бы в двух десятках пар «марка+модель».
