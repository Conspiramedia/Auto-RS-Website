...
export default async function proxy(request: NextRequest) {
  ...
  // ------------------------------------------------------------
  // ВОЗВРАТ ИЗ OAUTH: /auth/callback вне языковой машинерии.
  // ------------------------------------------------------------
  // Причина ровно та же, что у /admin выше, и ошибка была бы такой же
  // молчаливой. Адрес возврата прописан в консоли Google как
  // Authorized redirect URI и языкового префикса не имеет намеренно
  // (см. app/auth/callback/route.ts). Без этой ветки человека с cookie
  // 'ru' правило ниже увело бы редиректом на /ru/auth/callback,
  // которого не существует: вместо кабинета он получил бы 404 сразу
  // после успешного подтверждения входа у Google, причём одноразовый
  // код при этом уже был бы потрачен.
  //
  // Язык не теряется: он приезжает параметром ?locale= и определяет
  // зеркало, на которое обработчик вернёт человека после обмена кода.
  //
  // Сессию здесь НЕ продлеваем: её ещё нет — она появится через
  // несколько строк, в самом обработчике. Лишний getUser() означал бы
  // запрос к Auth заведомо без токена.
