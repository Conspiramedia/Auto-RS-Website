...
// умолчанию.
//
// Для СЕЛЕКТОРА ФОРМЫ берётся не этот список, а SELECTABLE_CONDITIONS
// ниже: 'normal' продавцу не показывается. Здесь он нужен, чтобы
// conditionStyle() умела ответить «оформления нет» на обычную машину,
// а не считать её неизвестным значением.
//
...
// значок — иначе сетка цветных плашек читалась бы как светофор.
...
// Состояния для СЕЛЕКТОРА ФОРМЫ: всё, кроме 'normal'.
//
// 'normal' в списке НЕ ПОКАЗЫВАЕТСЯ. Это значение по умолчанию, и
// отдельный пункт под него означал бы кнопку «оставить как есть» —
// продавец и так ничего не нажимает, если с машиной всё в порядке.
// Пункт при этом занимал бы первое, самое заметное место в сетке и
// перетягивал внимание с тех пяти состояний, ради которых поле и
// заведено.
//
// В БАЗЕ 'normal' остаётся: там это дефолт колонки и значение, к
// которому приводится всё неизвестное. Убрано оно только из
// интерфейса.
//
// Снять ошибочно поставленную пометку продавец может повторным
// нажатием на выбранный вариант — см. SellForm.
export const SELECTABLE_CONDITIONS = CAR_CONDITIONS.filter(
  (c) => c.key !== 'normal',
);
...
export type MyStatsTotals = {
  ...
  favorites: number;
  contacts: number;
  // Прирост за последние 7 суток (0141). Ноль означает «не
  // прибавилось».
  //
  // СЕЙЧАС В ИНТЕРФЕЙСЕ НЕ ПОКАЗЫВАЕТСЯ: подпись «+N за неделю» с
  // карточек метрик убрана по продуктовому решению. Поля оставлены,
  // потому что RPC их отдаёт, а тип обязан описывать её фактический
  // контракт — иначе следующий читатель решит, что функция вернула
  // лишнее. Данные при этом продолжают копиться, и вернуть показ
  // можно без миграции.
  //
  // ОГРАНИЧЕНИЯ, О КОТОРЫХ СТОИТ ЗНАТЬ, ЕСЛИ ПОКАЗ ВЕРНУТ:
  //   views_week считается по listing_view_log, а он ведётся только
  //     для авторизованных, поэтому цифра ЗАНИЖЕНА относительно
  //     прироста счётчика просмотров;
  //   contacts_week наполняется с момента применения 0141 — у
  //     объявлений, набравших контакты раньше, первую неделю будет 0.
  listings_week: number;
  views_week: number;
  favorites_week: number;
...
export type AdminCar = {
  ...
  archived_by: string | null;
  archived_reason: string | null;
  // Поля, доехавшие до карточки модерации в 0140. Каждое — предмет
  // проверки: объём сверяется с заголовком, доступность и состояние —
