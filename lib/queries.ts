...
export type CatalogFilters = {
  ...
  engineVolume?: string;
  // Показывать ли битые машины и разборку (миграция 0138).
  // Не задан или false — в выдаче только машины на ходу.
  // НЕ СВЯЗАН с availability салонов: это независимая ось, и «в пути»
  // остаётся в выдаче при любом значении флага.
  showDamaged?: boolean;
  sort?: SortKey;
  page?: number;
  perPage?: number;
...
export async function fetchCatalog(
  ...
  const { data, error } = await supabase.rpc('search_cars_public', {
    ...
    // СОСТОЯНИЕ: параметр добавляется, ТОЛЬКО когда флаг включён.
    // Причина та же, что у объёма и ленты ниже: supabase-js вызывает
    // RPC по именам, и лишний ключ означает другую сигнатуру — на базе
    // без миграции 0138 обычная выдача продолжит работать.
    //
    // Выключенный флаг ничего не передаёт, и битые отсекает СЕРВЕР
    // своим default false. Это и есть правильное умолчание: выдача без
    // доноров на разборку не должна зависеть от того, вспомнил ли
    // клиент прислать параметр.
    ...(filters.showDamaged ? { p_show_damaged: true } : {}),
