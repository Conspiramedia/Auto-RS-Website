...
// ------------------------------------------------------------
// Нормализация строки для поиска по списку.
// ------------------------------------------------------------
// Приводит к нижнему регистру и снимает диакритику, повторяя поведение
// f_normalize в PostgreSQL (unaccent + lower). Благодаря этому «uzice»
// находит «Užice», «skoda» — «Škoda», а «pancevo» — «Pančevo».
//
// Приложение использует обычный toLowerCase().contains(), из-за чего
// поиск «uzice» там ничего не находит. На сайте, где значение вводится
// с физической клавиатуры без сербской раскладки, это чувствуется
// сильнее, поэтому здесь нормализация полная.
export function normalizeForSearch(text: string): string {
  return text
    .normalize('NFD')
    // Диапазон комбинирующих диакритических знаков после NFD.
    .replace(/[̀-ͯ]/g, '')
    // Đ/đ не раскладываются через NFD — обрабатываются отдельно.
    .replace(/đ/g, 'd')
    .replace(/Đ/g, 'd')
    .toLowerCase()
    .trim();
