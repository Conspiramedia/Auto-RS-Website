...
export function buildMetadata(params: {
  ...
  //
  // ЛОКАЛЬ В АДРЕСЕ УЧАСТВУЕТ. Раньше сюда всегда подставлялся
  // корневой (сербский) роут: изображение считалось брендовым и
  // одинаковым для обоих языков. Но на превью есть слоган площадки, и
  // русские страницы отдавали его по-сербски.
  //
  // Ссылку пересылают с того зеркала, которое человек читает сам, —
  // язык адреса и есть сигнал о языке получателя. Поэтому /ru берёт
  // /ru/opengraph-image (app/ru/opengraph-image.tsx), а корень —
  // корневой; разметка и фон у них общие (lib/ogCover.tsx).
  const ogImage =
...
// ------------------------------------------------------------
// Состояние автомобиля в начале описания (миграция 0138).
// ------------------------------------------------------------
// «Битый Volkswagen Golf, 2015, Beograd…» вместо «Volkswagen Golf,
// 2015, Beograd…». Состояние стоит ПЕРВЫМ СЛОВОМ намеренно: сниппет в
// выдаче обрезается, и пометка, поставленная в конец, до читателя
// просто не доедет. Человек, ищущий целую машину, обязан отсеять
// объявление ещё в выдаче, а не после перехода.
//
// ДОСТУПНОСТЬ САЛОНА («в пути», «под заказ») сюда НЕ ПОПАДАЕТ, и это
// требование задачи, а не упущение: логистика меняется (машина
// приехала — пометка снялась), а переиндексация занимает недели, и
// описание в выдаче осталось бы врать. Состояние машины так не
