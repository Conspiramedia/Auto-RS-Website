...
export const dict = {
  sr: {
    ...
    // Карточка
    // Стоит НА МЕСТЕ СУММЫ — и в карточке, и на странице объявления,
    // и в кабинете. Слово «Cena» здесь избыточно: подпись и так
    // занимает позицию цены, а на узкой карточке лишнее слово
    // обрезалось многоточием.
    car_price_negotiable: 'Na upit',
    // Плейсхолдер поля цены при подаче. Совпадает по тексту с
    // car_price_negotiable выше, но ключ ОТДЕЛЬНЫЙ и объединять их не
    // следует: роли разные — там надпись на месте суммы в готовом
    // объявлении, здесь подсказка внутри пустого поля формы. Правка
    // одной формулировки не обязана менять вторую.
    ...
    my_totals_no_views_hint:
      'Dodajte 3+ fotografija i opis — takvi oglasi se češće gledaju.',
    ...
    // ------------------------------------------------------------
    // Заголовок главной идёт и в <title>, и в og:title —
    // buildMetadata собирает их из одного значения, поэтому вкладка
    // браузера и превью в мессенджере не разойдутся. Описание, в
    // отличие от него, разведено на две версии — см. ниже.
    //
    ...
    // meta_home_desc — в og:description и twitter:description.
    // Причина в разной жёсткости обрезки: Google режет сниппет
    // примерно на 160 символах и ставит многоточие, а мессенджеры
    // показывают описание целиком или обрезают заметно позже.
    //
    // Раньше оба тега брали одну строку, и выбор был безвыходным:
    // короткая теряла половину смысла в Telegram, длинная обрывалась
    // в выдаче на полуслове. Развести их — единственный способ дать
    // каждому каналу то, что он показывает полностью.
    //
    // Правя один ключ, проверьте второй: они должны говорить об одном
    // и том же, иначе поиск и мессенджер начнут описывать сайт
    // по-разному.
    meta_home_desc_short:
      'Oglasnik za prodaju i iznajmljivanje automobila u Srbiji. Privatni prodavci i saloni objavljuju besplatno; kupci biraju na srpskom i ruskom.',
    meta_home_desc:
  ...
  ru: {
    ...
    // См. комментарий в сербском словаре.
    car_price_negotiable: 'Договорная',
    ...
    sell_condition_hint:
      'Если с машиной всё в порядке — не выбирайте ничего. Повторное нажатие на выбранный вариант снимает выбор.',
    ...
    condition_note_parts:
      'Автомобиль на запчасти, не на ходу или восстановлению не подлежит.',
    condition_note_no_docs:
      'У автомобиля нет документов. Проверьте возможность регистрации.',
    condition_note_for_export:
      'Автомобиль продаётся только на экспорт из Сербии.',
    filter_show_damaged: 'Показать битые и в разборку',
    // См. комментарий в сербском словаре.
    ...
    meta_home_desc_short:
      'Маркетплейс по продаже и аренде авто в Сербии. Частные продавцы и автосалоны размещают бесплатно; покупатели выбирают на сербском и русском.',
    meta_home_desc:
