...
// ------------------------------------------------------------
// Удаление отклонённого объявления.
// ------------------------------------------------------------
// ОТДЕЛЬНОЕ ДЕЙСТВИЕ, а не ещё один статус в setCarStatus: удаление —
// не переход в матрице 0070, а выход из неё. Правило «удалять можно
// только отклонённое» живёт в RPC (0122), здесь его копии нет.
//
// У отклонённого объявления архив бессмыслен: вернуть оттуда в выдачу
// нельзя, модерация уже отказала, и запись оседала бы в кабинете
// навсегда. Поэтому вместо «Снять» продавцу предлагается удалить.
export async function deleteRejectedCar(
  carId: string,
): Promise<ActionResult> {
  const supabase = await getServerClient();

  const { error } = await supabase.rpc('delete_my_rejected_car', {
    p_car_id: carId,
  });
...
// ------------------------------------------------------------
// Письма о новых сообщениях: включить или выключить.
// ------------------------------------------------------------
// Отдельное действие, а не поле saveProfile: настройка применяется
// сразу по щелчку переключателя, без кнопки «Сохранить». Настройка
// уведомлений, которую нужно ещё и подтвердить отдельным нажатием,
// воспринимается как несработавшая.
//
// Флаг выключает ТОЛЬКО письма о сообщениях. Модерация объявления и
// код входа приходят всегда — см. комментарий в миграции 0132.
export async function setEmailOnMessage(
  enabled: boolean,
): Promise<{ ok: boolean }> {
  const supabase = await getServerClient();

  const { error } = await supabase.rpc('set_my_email_on_message', {
    p_enabled: enabled,
  });
...
//
// ПОДТВЕРЖДЕНИЕ идёт на сервер как есть: база сама решает, что должно
// было прийти — почта аккаунта, телефон или слово DELETE (0128), — и
// сверяет. Проверка в диалоге дублирует её ради быстрой реакции поля,
// но защитой не является: RPC можно позвать в обход интерфейса.
export type DeleteAccountResult = {
  ok: boolean;
  // Готовый к показу текст берёт клиент из своих локалей — сюда
  // возвращаем только РОД неудачи. Технические подробности Postgres
  // человеку, удаляющему аккаунт, ничего не объясняют.
  //
  // 'mismatch' отделён от 'unknown' намеренно: «почта не совпадает»
  // и «не удалось удалить» требуют от человека разных действий —
  // исправить ввод или попробовать позже. Одно сообщение на оба
  // случая заставляло бы гадать.
  error?: 'mismatch' | 'unknown';
};
...
export async function deleteAccount(
  ...
  if (error) {
    // check_violation бросается только несовпадением подтверждения:
    // остальные отказы функции идут с insufficient_privilege или
    // no_data_found. Значит по коду можно надёжно отличить ошибку
    // ввода от настоящего сбоя.
    if (error.code === '23514') return { ok: false, error: 'mismatch' };
