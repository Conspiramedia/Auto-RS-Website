...
const columns: Column<DealerLead>[] = [
  {
    ...
    render: (row) => (
      <div className="min-w-0">
        ...
        {/* Город под названием, а не отдельной колонкой: отдельная
            колонка под одно слово съела бы ширину у комментария.
            Проверка на null осталась: с 0103 город обязателен, но в
            таблице лежат заявки, поданные до неё. */}
        {row.city && (
          <p className="truncate text-micro text-neutral-50">{row.city}</p>
        )}
        {/* Реквизиты — здесь же, а не отдельными колонками: по ним
            админ проверяет компанию в APR, то есть читает их вместе с
            названием.
            tabular-nums выравнивает цифры по разрядам — сверять номер
            с выпиской так заметно легче.
            Проверка на null осталась ради заявок, поданных до 0103,
            когда реквизиты ещё не спрашивались. */}
  ...
  },
  {
    ...
    header: 'Почта',
    width: '190px',
    // hideBelow снят намеренно: почта обязательна с 0103 и остаётся
    // вторым способом связи, когда салон не берёт трубку. Прятать
    // заполненную у всех колонку до 1024px значило бы гонять админа
    // с планшета в карточку за каждым адресом.
    render: (row) =>
      row.email ? (
        <a
