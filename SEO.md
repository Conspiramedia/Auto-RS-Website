...
**Статус: правки внесены.** Документ описывает состояние ПОСЛЕ исправления
флагов — таблицы приведены к фактическим значениям, раздел 7 отмечает по
каждой проблеме, что с ней сделано. Длины проверены на отдаваемом HTML
(`next start` + curl), а не только по исходникам.
...
| index | index | index |
| JSON-LD | `HowTo` + 3 × `HowToStep` ✅ | `HowTo` + 3 × `HowToStep` ✅ |
| og:title / og:description | = title / = description | = title / = description |
...
|---|---|---|
| title raw / `<head>` | `Uslovi korišćenja platforme RS Auto` — 35 / **45** ✅ | `Условия использования платформы RS Auto` — 39 / **49** ✅ |
...
заголовке блока на главной. «Uslovi korišćenja platforme RS Auto» в
подвале повторяло бы бренд рядом с брендом, а `rent_title` длиной 32
символа поехал бы в крошки четырёх компонентов.
...
**Замечание, требующее вашего решения.** Новый заголовок `/terms` даёт в
разметке `Uslovi korišćenja platforme RS Auto | RS Auto` — бренд дважды
подряд. Текст взят из задания дословно, поэтому оставлен как есть; если
дублирование нежелательно, лечится либо укорачиванием строки
(`Uslovi korišćenja` + суффикс), либо `title: { absolute }` по образцу
главной. То же в русской версии.
...
### 14. `<h1>` на витринах — по-прежнему не проверялось

Инвентаризация охватывает `<head>`. Соответствие `<h1>` заголовку и
уникальность заголовков внутри витрин марок и моделей — предмет отдельной
ревизии.
...
| title < 30 | 8 публичных типов | нет среди индексируемых |
...
| `og:type` карточки | website | website — Next 16 не поддерживает product |
| `product:price:*` | не было | есть |
| JSON-LD `/sell`, `/privacy`, `/terms` | отсутствовал | `HowTo`, `WebPage`, `WebPage` |

### Открытые вопросы к вам

1. **`/terms`: бренд дважды** — `Uslovi korišćenja platforme RS Auto |
   RS Auto`. Текст из задания дословный; убрать дубль можно укорачиванием
   строки или `absolute`-заголовком.
2. **`og:type=product`** — средствами Next 16 недостижим (разбор в пункте
   12). Нужен ли обходной путь через ручной `<meta>` в теле страницы.
